...
namespace Corporation
{
    public partial class Form5 : Form
    {
        ...
        private void LoadHeader()
        {
            ...
            txtCurrentJobLevel.Text = currentPosition.job.job_level.name ?? "-";
        }

        private void LoadAvailablePromotionJobs()
        {
            var currentPosition = db.positions
                .Where(p => p.employee_id == _employeeId && p.deleted_at == null)
                .OrderByDescending(p => p.created_at)
            ...
            int targetLevelId = currentLevelId + 1;

            var appliedJobIds = db.promotions
                .Where(m => m.deleted_at == null && m.employee_id == _employeeId && m.status == "Pending")
                .Select(m => m.job_id)
                .ToList();

            var available = db.jobs
                .Where(j => j.deleted_at == null && j.job_level_id == targetLevelId && j.id != currentJobId)
                .Select(j => new
                {
                    JobId = j.id,
            ...
            {
                MessageBox.Show("No available promotion position.");
            }
        }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            MessageBox.Show("Promotion applied (Pending).");
            LoadAvailablePromotionJobs();
        }
